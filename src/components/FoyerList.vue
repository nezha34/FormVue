        ...
        <div class="search-panels">
            <div class="search-panel">
                <div class="search-input-wrapper">
                    ...
                    <input v-model="searchFoyerId" type="text" placeholder="Rechercher par ID Foyer"
                        class="search-input" />
                </div>
            </div>
            <div class="search-panel">
                <div class="search-input-wrapper">
                    ...
                    <input v-model="searchGroup" type="text" placeholder="Rechercher par Groupe" class="search-input" />
                </div>
            </div>
        </div>
                ...
                <tr v-for="foyer in paginatedFoyers" :key="foyer.no_foyer">
                    <td>
                        <a href="#" @click.prevent="selectFoyer(foyer)" class="foyer-link">
                            ...
                            <Chip v-if="foyer.locked_by"
                                :icon="foyer.is_rendezvous ? 'pi pi-calendar-clock' : 'pi pi-lock'" class="p-chip"
                                :class="foyer.is_rendezvous ? 'p-chip-success' : 'p-chip-warning'"
...
const fetchFoyers = async () => {
    try {
        ...
        foyersList.value = response.data.map((foyer) => ({
            ...foyer,
            locked_by: foyer.locked_by || null, // Ensure locked status is accounted for
        }));
    } catch (err) {
        toast.add({
            severity: 'error',
...
const fetchobjective = async () => {
    try {
        ...
        toast.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Échec de la récupération des objectifs',
            life: 3000
        });
    }
};
...
const selectFoyer = async (foyer) => {
    try {
        await axios.patch(`http://127.0.0.1:8000/foyers/${foyer.no_foyer}/lock?agent_id=${props.agent}`);
        emit('foyer-selected', foyer.no_foyer);
        foyersList.value = foyersList.value.filter(f => f.no_foyer !== foyer.no_foyer);
    } catch (error) {
        if (error.response && error.response.status === 409) {
            // Foyer is locked by another agent
            toast.add({
                severity: 'error',
                summary: 'Error',
